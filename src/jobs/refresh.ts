...
// Nothing is instantiated at module level — safe to import in serverless.
// Queue and Worker are created only when scheduleRefreshJobs() is called,
// which src/index.ts does but api/index.ts never does.
...
export function scheduleRefreshJobs() {
  ...
  queue = new Queue("oddy-refresh", { connection });

  queue.add("leaderboard:refresh", {}, { repeat: { every: 4 * 60 * 1000 } });
  queue.add("trending:refresh",    {}, { repeat: { every: 2 * 60 * 1000 } });
  queue.add("stats:refresh",       {}, { repeat: { every: 45 * 1000 } });
